...
度量对象：GPT‑4o、Claude3．5 Sonnet、Gemini2．0 Flash、o1‑preview。
...
Claude3．5：无。其自我反思行为仍由提示链触发，非内生。
...
Claude3．5：高。逻辑一致性优于GPT‑4o。长文本推理稳定性强。

Gemini2．0：中高。推理能力存在波动。同一问题不同次回答可能不一致。
...
Claude3．5：中高。对不知道的识别优于GPT‑4o。会在不确定时主动声明。但仍存在看似知道实则不知的情况。

Gemini2．0：中。边界识别能力与GPT‑4o近似。
...
Claude3．5：低中。同上。在明确提示请解释因果时，可生成因果链表述，但该表述本身仍来自训练数据的因果语言模式。
...
Claude3．5：高。意图理解能力在四者中最强。能处理复杂、模糊、多层隐含意图。

Gemini2．0：中。意图理解存在波动。
...
Claude3．5：高。长上下文持恒能力最优。200K上下文窗口内稳定性强。

Gemini2．0：中高。超长上下文能力存在，但稳定性不及Claude3．5。
...
Claude3．5：弱中。Constitutional AI流程中包含自我批评环节，但批评与生成仍在同一模型内，非真正独立。
