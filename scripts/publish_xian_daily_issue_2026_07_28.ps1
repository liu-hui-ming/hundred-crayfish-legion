...
<#
  XIAN one-post-per-day template (2026-07-28). Title/body from UTF-8 files (no Chinese in this .ps1).
  Labels: P1-Roadmap, documentation

  $env:GH_TOKEN = "ghp_..."
  powershell -ExecutionPolicy Bypass -File .\scripts\publish_xian_daily_issue_2026_07_28.ps1
#>
param(
    [string] $Token,
...
$titlePath = Join-Path $root "docs\issue-exports\xian-daily-2026-07-28-title.txt"
$bodyPath = Join-Path $root "docs\issue-exports\xian-daily-2026-07-28-body.md"
...
$headers = @{
    ...
    "User-Agent"           = "HCL-publish-xian-daily-20260728-ps1"
}
...
if ($WhatIf) {
    Write-Host "WhatIf: POST issue (2026-07-28 XIAN daily template)"
    exit 0
}
...
$marker = "## REGISTRY_XIAN_DAILY_2026_07_28"
...
if ($regFull -notmatch [regex]::Escape($marker)) {
    ...
    $row = '| #' + $n + ' | XIAN daily + P2 archive + four-module observation + three-ledger sync (2026-07-28) | ' + $url + ' |'
    $append =
        $nl + $nl + '---' + $nl + $nl +
        $marker + $nl + $nl +
        '**Posted:** 2026-07-28' + $nl + $nl +
        '| Issue | Role | Link |' + $nl +
        '|-------|------|------|' + $nl +
        $row + $nl + $nl +
        'Title file: `docs/issue-exports/xian-daily-2026-07-28-title.txt`' + $nl +
        'Body file: `docs/issue-exports/xian-daily-2026-07-28-body.md`' + $nl +
        'Labels: `P1-Roadmap`, `documentation`' + $nl +
        'Script: `scripts/publish_xian_daily_issue_2026_07_28.ps1`' + $nl
    ...
    if ($regFull -notmatch [regex]::Escape($issueNeedle)) {
        $nl = [Environment]::NewLine
        $row = '- Follow-up entry: #' + $n + '  ' + $url + '  (Posted: 2026-07-28)'
